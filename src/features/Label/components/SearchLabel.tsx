...
export default function SearchLabel(props: SearchLabelProps) {
  ...
  const getLabelsQuery = useQuery<getLabelsType, getLabelsErrorType>(
    "labels",
    getLabels,
    {
      staleTime: Infinity,
  ...
  const labels =
    searchValue !== "" && searchLabelMutation.isSuccess
  ...
  return (
    <Modal visible={props.visible} toggle={props.toggle}>
      ...
      {getLabelsQuery.isError && (
        <p className="text-center p-3 text-red-500">
          {getLabelsQuery.error?.message}
        </p>
      )}
      {getLabelsQuery.isLoading || searchLabelMutation.isLoading ? (
        ...
        <div className="px-3 max-h-96 overflow-y-auto">
          {labels != null && (
            <SearchLabelList labels={labels} noteId={props.noteId} />
          )}
        </div>
      )}
    </Modal>
...
function SearchLabelItem({ label, noteId }: { label: Label; noteId: string }) {
  ...
  const addLabelMutation = useMutation(
    () => addLabel({ labelId: label.id, noteId }),
    { onSuccess }
  );

  const removeLabelMutation = useMutation(
    () => removeLabel({ labelId: label.id, noteId }),
    { onSuccess }
  );
