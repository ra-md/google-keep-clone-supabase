...
export default function CreateNote(props: CreateNoteProps) {
	...
	const { mutate } = useMutation<Note[] | null, unknown, CreateNoteData>((noteData) => createNote(noteData), {
		onSuccess() {
	...
	return (
		<>
			...
			>
				...
				{
					visible && <div className='m-3 flex justify-end'>
						<Button className='font-semibold' aria-label='close create note' size='small' onClick={submit}>Close</Button>
					</div>
				}
			</div>
		</>
