...
export default function NoteLabels({
  noteLabels,
  noteId,
  full = false,
}: NoteLabelsProps) {
  ...
  return (
    <div className="flex">
      <ul className="relative flex-wrap flex z-10">
        {labels.map((label) => (
          <NoteLabelsItem key={label.id} noteId={noteId} {...label} />
        ))}
      </ul>
      {noteLabels.length > 1 && !full && (
        <span className="rounded-md border border-secondary px-2 text-sm">
          +{noteLabels.length - 1}
        </span>
      )}
    </div>
...
function NoteLabelsItem(props: Label & { noteId: string }) {
  ...
  const deleteLabelMutation = useMutation(
    () => removeLabel({ labelId: props.id, noteId: props.noteId }),
    {
      onSuccess() {
